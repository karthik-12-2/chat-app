    ...
    >
      ...
      <Box display="flex" alignItems="center">
        ...
        <Box className="ms-3 pt-1">
          ...
          {groupMembers?.map(gm => 
          <Typography variant="body2" color="success.main" key={gm.userId}>{`${gm.userName}, `}</Typography>
        )}
        </Box>
        </Box>
      </Box>
      ...
      <Box display="flex" justifyContent="center" alignItems="center" gap={2}>
        ...
        <VideocamIcon onClick={() => {handleClick('video')}} sx={{cursor: 'pointer'}}/>
        <MoreVertIcon onClick={() => {handleClick('more')}} sx={{cursor: 'pointer'}}/>
      </Box>
    </Box>
  );
};
